...
"""
Welcome to PyTracer Module 

This module provides a versatile tool, PyTracer, for tracing the execution of Python code, capturing variable changes,
and logging print statements. PyTracer helps you gain insights into the sequence of code execution, variable values, and
...
Usage:
    To use the PyTracer module, instantiate the CodeTracer class with the Python code you want to trace. You can then use
...
class CodeTracer(object):
    ...
    def __str__(self) -> str:
        ...
        return print(
            tabulate.tabulate(self.trace_table, headers="firstrow", tablefmt="grid")
        )

    def format_code_for_tracing(self):
        """
    ...
    def variables_tracer(
        ...
        if event == "call":
            ...
            vars_in_context = {
                f"({frame.f_code.co_name}){var}": (
                    (tabulate.tabulate([vars(value).keys(), vars(value).values()], tablefmt="grid")).replace("\n","<br>") 
                    if "object" in str(value) and var != "self" else str(value)
                )
